...
class CustomGymEnv(GymEnv):
    ...
    def close(self):
        ...
        logger.debug(f"Closing CustomGymEnv (parent: {super().__class__.__name__})")
        # Close the init_env if it exists and hasn't been closed
        if hasattr(self, "init_env") and self.init_env is not None:
            try:
                # Check if the underlying env has a close method
                if hasattr(self.init_env, "close") and callable(self.init_env.close):
                     self.init_env.close()
                logger.debug("Closed init_env.")
            except Exception as e:
                logger.warning(f"Error closing init_env: {e}")
            # Set to None regardless of successful close to prevent reuse
            self.init_env = None

        # Close observation space if it exists and has a close method
        if hasattr(self, "observation_space") and self.observation_space is not None:
            if hasattr(self.observation_space, "close") and callable(self.observation_space.close):
                try:
                    self.observation_space.close()
                    logger.debug("Closed observation_space.")
                except Exception as e:
                     logger.warning(f"Error closing observation_space: {e}")
            # Set to None regardless
            self.observation_space = None

        # Close action space if it exists and has a close method (Handles the error)
        if hasattr(self, "action_space") and self.action_space is not None:
             # Check if the action_space *itself* has close, not its converter
             if hasattr(self.action_space, "close") and callable(self.action_space.close):
                  try:
                      self.action_space.close()
                      logger.debug("Closed action_space.")
                  except Exception as e:
                       logger.warning(f"Error closing action_space: {e}")
             else:
                  logger.debug(f"Action space of type {type(self.action_space)} does not have a close method. Skipping.")
             # Set to None regardless
             self.action_space = None

        # Ensure parent close is called if necessary (depends on parent implementation)
        # super().close() # Might be needed if GymEnv has essential close logic

        logger.debug("CustomGymEnv closed.")
    # --- Add method end ---
